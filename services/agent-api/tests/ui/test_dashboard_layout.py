...
def test_dashboard_has_project_regions_detail_drawer_and_persistent_chat() -> None:
    ...
    assert (
        "@media(max-width:620px){.topbar,.project-controls{align-items:stretch;flex-direction:column}.topbar{padding:1rem}.dashboard-shell{padding:.5rem}.dashboard,.dashboard-lower{grid-template-columns:1fr}.summary-cards{grid-template-columns:1fr}"
        in normalized
    )
