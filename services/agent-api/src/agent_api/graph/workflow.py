from __future__ import annotations

import asyncio
import logging
import os
import time
from typing import Protocol, cast

from langgraph.graph import END, START, StateGraph
from workforce_risk.models import RiskResult

from agent_api.dashboard.models import DashboardSnapshot
from agent_api.evidence.collectors import (
    ProgressHistoryReader,
    UniversalEvidenceCollector,
)
from agent_api.evidence.focus import answer_context_from_evidence
from agent_api.evidence.models import (
    EvidenceCategory,
    EvidencePlan,
    MissingData,
    UniversalEvidenceBundle,
)
from agent_api.evidence.normalizer import normalize_evidence_plan
from agent_api.evidence.planner import conservative_read_plan
from agent_api.evidence.references import resolve_conversation_references
from agent_api.failure_diagnostics import classify_failure, log_assistant_event
from agent_api.graph.intents import Intent
from agent_api.graph.state import (
    EntityReferences,
    GraphState,
    InvestigationResponse,
    VerifiedAgentContext,
)
from agent_api.graph.supervisor import (
    capability_response,
    classify_supported_intent,
    receive_verified_context,
    route_after_classification,
)
from agent_api.historical_evidence import HistoricalRiskContext
from agent_api.llm.protocol import ExplanationProvider
from agent_api.llm.schemas import ExplanationRequest
from agent_api.risk_evidence import RiskEvidenceDossier
from agent_api.task_queries import GroundedAnswerContext, TaskQueryResult

logger = logging.getLogger(__name__)

_UNSUPPORTED_TASK_QUERY_ERRORS = frozenset(
    {
        "employee is required for a deadline task query",
        "Jira task queries require Jira evidence mode",
        "only an explicit tomorrow deadline is supported",
        "unsupported Jira task query question",
    }
)

_OPTIONAL_DOMAIN_EVIDENCE_ERRORS = frozenset(
    {
        "the requested deterministic domain tool is unavailable",
        "employee_id is required for overload investigation",
    }
)


class InvestigationTool(Protocol):
    async def investigate(
        self,
        intent: Intent,
        references: EntityReferences,
        correlation_id: str,
    ) -> RiskResult: ...


class TaskQueryTool(Protocol):
    async def query(
        self,
        question: str,
        references: EntityReferences,
        correlation_id: str,
        *,
        previous_context: GroundedAnswerContext | None = None,
    ) -> TaskQueryResult: ...


class RiskDossierTool(Protocol):
    async def get_current_dossier(
        self, employee_id: str, project_key: str, correlation_id: str
    ) -> RiskEvidenceDossier: ...


class HistoricalEvidenceTool(Protocol):
    async def get_at(
        self,
        question: str,
        employee_id: str,
        project_key: str,
        correlation_id: str,
    ) -> HistoricalRiskContext: ...


class ProjectEvidenceTool(Protocol):
    async def build(
        self, project_key: str, correlation_id: str
    ) -> DashboardSnapshot: ...


class InvestigationWorkflow:
    def __init__(
        self,
        *,
        tool: InvestigationTool,
        task_query_tool: TaskQueryTool | None = None,
        dossier_tool: RiskDossierTool | None = None,
        historical_tool: HistoricalEvidenceTool | None = None,
        project_tool: ProjectEvidenceTool | None = None,
        progress_history_reader: ProgressHistoryReader | None = None,
        explainer: ExplanationProvider,
        max_steps: int = 6,
        max_tool_calls: int = 2,
        deadline_seconds: float | None = None,
    ) -> None:
        self._tool = tool
        self._task_query_tool = task_query_tool
        self._dossier_tool = dossier_tool
        self._historical_tool = historical_tool
        self._project_tool = project_tool
        self._progress_history_reader = progress_history_reader
        self._explainer = explainer
        self._max_steps = max_steps
        self._max_tool_calls = max_tool_calls
        configured_deadline = (
            deadline_seconds
            if deadline_seconds is not None
            else float(os.getenv("CHAT_WORKFLOW_DEADLINE_SECONDS", "0"))
        )
        self._deadline_seconds = (
            configured_deadline if configured_deadline > 0 else None
        )
        builder = StateGraph(GraphState)
        builder.add_node("receive_verified_context", receive_verified_context)
        builder.add_node("classify", classify_supported_intent)
        builder.add_node("plan_evidence", self._plan_evidence)
        builder.add_node("collect_evidence", self._collect_evidence)
        builder.add_node("gather", self._gather)
        builder.add_node("explain", self._explain)
        builder.add_node("capability", capability_response)
        builder.add_edge(START, "receive_verified_context")
        builder.add_edge("receive_verified_context", "classify")
        if self._project_tool is not None:
            builder.add_edge("classify", "plan_evidence")
            builder.add_edge("plan_evidence", "collect_evidence")
            builder.add_edge("collect_evidence", "explain")
        else:
            builder.add_conditional_edges(
                "classify",
                route_after_classification,
                {"gather": "gather", "capability": "capability"},
            )
        builder.add_edge("gather", "explain")
        builder.add_edge("explain", END)
        builder.add_edge("capability", END)
        self._graph = builder.compile()

    def _missing_optional_evidence(
        self,
        state: GraphState,
        *,
        category: EvidenceCategory,
        reason: str,
        attempted_tool_calls: int = 0,
    ) -> dict[str, object]:
        correlation_id = state["verified_context"].correlation_id
        bundle = UniversalEvidenceBundle(
            correlation_id=correlation_id,
            project_key=state["references"].project_key,
            plan=conservative_read_plan(),
            missing_data=(
                MissingData(
                    category=category,
                    reason=reason,
                    entity=state["references"].project_key,
                    required_for_claim=True,
                ),
            ),
        )
        log_assistant_event(
            logger,
            "optional_evidence_unavailable",
            correlation_id,
            intent=state["intent"].value,
            reason=reason,
        )
        return {
            "universal_evidence": bundle,
            "steps_used": state["steps_used"] + 1,
            "tool_calls_used": state["tool_calls_used"] + attempted_tool_calls,
        }

    async def _plan_evidence(self, state: GraphState) -> dict[str, object]:
        correlation_id = state["verified_context"].correlation_id
        log_assistant_event(
            logger, "langgraph_node_entered", correlation_id, node="plan_evidence"
        )
        planner = getattr(self._explainer, "plan_evidence", None)
        started = time.perf_counter()
        log_assistant_event(logger, "bedrock_planning_started", correlation_id)
        try:
            plan = (
                await planner(
                    question=state["question"],
                    project_key=state["references"].project_key,
                    employee_id=state["references"].employee_id,
                    previous_context=state.get("previous_answer_context"),
                    correlation_id=correlation_id,
                )
                if planner is not None
                else conservative_read_plan()
            )
        except Exception as exc:
            log_assistant_event(
                logger,
                "bedrock_planning_finished",
                correlation_id,
                duration_ms=round((time.perf_counter() - started) * 1000),
                success=False,
                failure_code=classify_failure(exc, component="bedrock"),
                exception_type=type(exc).__name__,
                exception_message=str(exc),
            )
            raise
        log_assistant_event(
            logger,
            "bedrock_planning_finished",
            correlation_id,
            duration_ms=round((time.perf_counter() - started) * 1000),
            success=True,
        )
        typed_plan = EvidencePlan.model_validate(plan)
        original_categories = [item.value for item in typed_plan.evidence_categories]
        resolved = resolve_conversation_references(
            state["question"], state.get("previous_answer_context")
        )
        if resolved.entities and not typed_plan.entities:
            typed_plan = typed_plan.model_copy(update={"entities": resolved.entities})
        typed_plan = normalize_evidence_plan(
            question=state["question"],
            planned=typed_plan,
            previous_context=state.get("previous_answer_context"),
        )
        log_assistant_event(
            logger,
            "evidence_plan_normalized",
            correlation_id,
            original_categories=original_categories,
            normalized_categories=[
                item.value for item in typed_plan.evidence_categories
            ],
            normalized_scope=typed_plan.scope.value,
        )
        result: dict[str, object] = {
            "evidence_plan": typed_plan,
            "steps_used": state["steps_used"] + 1,
        }
        if resolved.needs_clarification:
            result["ambiguous_references"] = resolved.entities
        return result

    async def _collect_evidence(self, state: GraphState) -> dict[str, object]:
        correlation_id = state["verified_context"].correlation_id
        log_assistant_event(
            logger, "langgraph_node_entered", correlation_id, node="collect_evidence"
        )
        if self._project_tool is None:
            raise RuntimeError("required project evidence service is unavailable")
        if state["tool_calls_used"] >= self._max_tool_calls:
            raise RuntimeError("investigation tool-call limit reached")
        bundle = await UniversalEvidenceCollector(
            self._project_tool,
            history_reader=self._progress_history_reader,
        ).collect(
            state["evidence_plan"],
            project_key=state["references"].project_key,
            correlation_id=correlation_id,
            source_employee_id=state["references"].employee_id,
            question=state["question"],
            previous_context=state.get("previous_answer_context"),
        )
        if state.get("ambiguous_references"):
            bundle = bundle.model_copy(
                update={"ambiguous_references": state["ambiguous_references"]}
            )
        log_assistant_event(
            logger,
            "universal_evidence_collected",
            correlation_id,
            categories=[
                item.value for item in state["evidence_plan"].evidence_categories
            ],
            issue_count=len(
                bundle.project_snapshot.tasks if bundle.project_snapshot else ()
            ),
            employee_count=len(
                bundle.project_snapshot.employees if bundle.project_snapshot else ()
            ),
            missing_data_count=len(bundle.missing_data),
        )
        return {
            "universal_evidence": bundle,
            "project_snapshot": bundle.project_snapshot,
            "steps_used": state["steps_used"] + 1,
            "tool_calls_used": state["tool_calls_used"] + 1,
        }

    async def _gather(self, state: GraphState) -> dict[str, object]:
        correlation_id = state["verified_context"].correlation_id
        log_assistant_event(
            logger, "langgraph_node_entered", correlation_id, node="gather"
        )
        if state["tool_calls_used"] >= self._max_tool_calls:
            raise RuntimeError("investigation tool-call limit reached")
        if state["intent"] is Intent.EXPLAIN_PROJECT_RISK:
            if self._project_tool is None:
                return self._missing_optional_evidence(
                    state,
                    category=EvidenceCategory.JIRA_ISSUES,
                    reason="Current project evidence is unavailable.",
                )
            snapshot = await self._project_tool.build(
                state["references"].project_key,
                state["verified_context"].correlation_id,
            )
            log_assistant_event(
                logger,
                "evidence_bundle_selected",
                correlation_id,
                sources=["jira_mcp", "workforce_risk_mcp"],
                issue_count=len(snapshot.tasks),
                employee_count=len(snapshot.employees),
            )
            return {
                "project_snapshot": snapshot,
                "steps_used": state["steps_used"] + 1,
                "tool_calls_used": state["tool_calls_used"] + 1,
            }
        if state["intent"] is Intent.JIRA_TASK_QUERY:
            if self._task_query_tool is None:
                return self._missing_optional_evidence(
                    state,
                    category=EvidenceCategory.JIRA_ISSUES,
                    reason="Current structured Jira task evidence is unavailable.",
                )
            started = time.perf_counter()
            log_assistant_event(
                logger,
                "jira_tool_called",
                correlation_id,
                tool="deterministic_task_query",
            )
            try:
                result = await self._task_query_tool.query(
                    state["question"],
                    state["references"],
                    state["verified_context"].correlation_id,
                    previous_context=state.get("previous_answer_context"),
                )
                log_assistant_event(
                    logger,
                    "jira_tool_finished",
                    correlation_id,
                    tool="deterministic_task_query",
                    duration_ms=round((time.perf_counter() - started) * 1000),
                    success=True,
                )
            except ValueError as exc:
                if str(exc) not in _UNSUPPORTED_TASK_QUERY_ERRORS:
                    raise
                if self._project_tool is None:
                    return self._missing_optional_evidence(
                        state,
                        category=EvidenceCategory.JIRA_ISSUES,
                        reason="Current structured Jira task evidence is unavailable.",
                        attempted_tool_calls=1,
                    )
                if state["tool_calls_used"] + 1 >= self._max_tool_calls:
                    raise RuntimeError("investigation tool-call limit reached") from exc
                snapshot = await self._project_tool.build(
                    state["references"].project_key,
                    state["verified_context"].correlation_id,
                )
                return {
                    "intent": Intent.EXPLAIN_PROJECT_RISK,
                    "project_snapshot": snapshot,
                    "steps_used": state["steps_used"] + 1,
                    "tool_calls_used": state["tool_calls_used"] + 2,
                }
            except Exception as exc:
                log_assistant_event(
                    logger,
                    "jira_tool_finished",
                    correlation_id,
                    tool="deterministic_task_query",
                    duration_ms=round((time.perf_counter() - started) * 1000),
                    success=False,
                    failure_code=classify_failure(exc, component="jira"),
                    exception_type=type(exc).__name__,
                    exception_message=str(exc),
                )
                raise
            typed_result = TaskQueryResult.model_validate(result)
            log_assistant_event(
                logger,
                "evidence_bundle_selected",
                correlation_id,
                sources=["jira_mcp"],
                jira_result_count=len(typed_result.tasks),
                issue_count=len(typed_result.tasks),
            )
            return {
                "task_query_result": typed_result,
                "steps_used": state["steps_used"] + 1,
                "tool_calls_used": state["tool_calls_used"] + 1,
            }
        if state["intent"] is Intent.EXPLAIN_HISTORY:
            if self._historical_tool is None or state["references"].employee_id is None:
                return self._missing_optional_evidence(
                    state,
                    category=EvidenceCategory.HISTORY,
                    reason="Historical employee evidence is unavailable.",
                )
            historical = await self._historical_tool.get_at(
                state["question"],
                state["references"].employee_id,
                state["references"].project_key,
                state["verified_context"].correlation_id,
            )
            historical_result: dict[str, object] = {
                "risk": historical.risk,
                "evidence_dossier": historical.dossier,
                "steps_used": state["steps_used"] + 1,
                "tool_calls_used": state["tool_calls_used"] + 1,
            }
            if historical.previous_dossier is not None:
                historical_result["previous_evidence_dossier"] = (
                    historical.previous_dossier
                )
            return historical_result
        try:
            risk = await self._tool.investigate(
                state["intent"],
                state["references"],
                state["verified_context"].correlation_id,
            )
        except ValueError as exc:
            if str(exc) not in _OPTIONAL_DOMAIN_EVIDENCE_ERRORS:
                raise
            return self._missing_optional_evidence(
                state,
                category=EvidenceCategory.RISK_RESULTS,
                reason=(
                    "The requested optional domain evidence is not available "
                    "in the current evidence mode."
                ),
                attempted_tool_calls=1,
            )
        gathered: dict[str, object] = {
            "risk": RiskResult.model_validate(risk),
            "steps_used": state["steps_used"] + 1,
            "tool_calls_used": state["tool_calls_used"] + 1,
        }
        if (
            state["intent"] is Intent.EXPLAIN_EMPLOYEE_OVERLOAD
            and self._dossier_tool is not None
            and state["references"].employee_id is not None
        ):
            if state["tool_calls_used"] + 1 >= self._max_tool_calls:
                raise RuntimeError("investigation tool-call limit reached")
            gathered["evidence_dossier"] = await self._dossier_tool.get_current_dossier(
                state["references"].employee_id,
                state["references"].project_key,
                state["verified_context"].correlation_id,
            )
            gathered["tool_calls_used"] = state["tool_calls_used"] + 2
        return gathered

    async def _explain(self, state: GraphState) -> dict[str, object]:
        correlation_id = state["verified_context"].correlation_id
        log_assistant_event(
            logger, "langgraph_node_entered", correlation_id, node="explain"
        )
        started = time.perf_counter()
        log_assistant_event(
            logger,
            "bedrock_request_started",
            correlation_id,
            bedrock_invoked=True,
            model_id=os.getenv("BEDROCK_MODEL_ID", "not-configured"),
        )
        try:
            explanation = await self._explainer.explain(
                ExplanationRequest(
                    workflow=state["intent"].value,
                    question=state["question"],
                    risk=state.get("risk"),
                    project_snapshot=state.get("project_snapshot"),
                    task_query_result=state.get("task_query_result"),
                    evidence_dossier=state.get("evidence_dossier"),
                    previous_evidence_dossier=state.get("previous_evidence_dossier"),
                    previous_answer_context=state.get("previous_answer_context"),
                    universal_evidence=state.get("universal_evidence"),
                    correlation_id=state["verified_context"].correlation_id,
                )
            )
        except Exception as exc:
            log_assistant_event(
                logger,
                "bedrock_request_finished",
                correlation_id,
                duration_ms=round((time.perf_counter() - started) * 1000),
                success=False,
                failure_code=classify_failure(exc, component="bedrock"),
                exception_type=type(exc).__name__,
                exception_message=str(exc),
            )
            raise
        log_assistant_event(
            logger,
            "bedrock_request_finished",
            correlation_id,
            duration_ms=round((time.perf_counter() - started) * 1000),
            success=True,
            bedrock_success=True,
        )
        return {
            "explanation": explanation,
            "steps_used": state["steps_used"] + 1,
        }

    async def run(
        self,
        *,
        verified_context: VerifiedAgentContext,
        question: str,
        references: EntityReferences,
        previous_answer_context: GroundedAnswerContext | None = None,
    ) -> InvestigationResponse:
        if len(question) > 1_000:
            raise ValueError("question exceeds size limit")
        initial: GraphState = {
            "verified_context": verified_context,
            "question": question,
            "references": references,
            "steps_used": 0,
            "tool_calls_used": 0,
        }
        if previous_answer_context is not None:
            initial["previous_answer_context"] = previous_answer_context
        invocation = self._graph.ainvoke(initial, {"recursion_limit": self._max_steps})
        raw = (
            await invocation
            if self._deadline_seconds is None
            else await asyncio.wait_for(invocation, timeout=self._deadline_seconds)
        )
        final = cast(GraphState, raw)
        return InvestigationResponse(
            correlation_id=verified_context.correlation_id,
            intent=final["intent"],
            risk=final.get("risk"),
            explanation=final.get("explanation"),
            capability_guidance=final.get("capability_guidance"),
            missing_sources=final.get("missing_sources", ()),
            steps_used=final["steps_used"],
            tool_calls_used=final["tool_calls_used"],
            answer_context=(
                final["task_query_result"].context
                if final.get("task_query_result") is not None
                else answer_context_from_evidence(
                    final["universal_evidence"].answer_evidence
                )
                if final.get("universal_evidence") is not None
                and final["universal_evidence"].answer_evidence is not None
                else None
            ),
        )
