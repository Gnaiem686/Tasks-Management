...
**Architecture:** A lightweight HTML/CSS/JavaScript manager UI is served directly by the API-key-protected FastAPI/LangGraph Agent API. Every normal chat request resolves structured references, obtains a validated Bedrock evidence-category plan, runs mapped allowlisted read-only collectors, derives deterministic calculations/rankings, and supplies an optional-safe UniversalEvidenceBundle to Bedrock for final prose. Fixed intents are telemetry or optional optimization hints only. MCP servers remain tools, Workforce Risk MCP retains deterministic logic and transactional state, PostgreSQL stores authoritative workflow state and profiles, Jira Cloud stores work data, S3 stores reports, and all custom workloads run in `dev` and `prod` namespaces on the Terraform-provisioned kubeadm cluster.
...
- Every successful user-visible chat answer is plain text generated by Bedrock.
  Deterministic code gathers and validates evidence, calculates scores, resolves
  references, selects tools, and enforces safety, but never writes final chat
  prose. After a bounded server attempt is exhausted by a retryable Bedrock
  failure, the browser keeps the turn pending and retries until Bedrock answers
  or the manager cancels/leaves. Permanent failures return a safe
  correlation-ID error. There is no deterministic chat-text fallback. Bedrock
  never creates or alters scores,
  candidates, approvals, or proposal state.
- Normal chat cannot be rejected or routed exclusively by a finite intent
  whitelist. Fixed intents are non-authoritative telemetry/analytics hints.
  The authoritative flow is reference resolution -> EvidencePlan -> allowlisted
  collectors -> UniversalEvidenceBundle -> deterministic derivation -> Bedrock
  -> generic grounding/completeness.
- Missing optional data or capability is typed `missing_data`, not an exception.
  Missing employee ID, skills, estimate, history, candidate, unknown wording, or
  an ambiguous reference must not produce an infrastructure-unavailable 503.
- Capacity terms are exact: headroom is configured capacity minus workload,
  available capacity is `max(headroom, 0)`, and utilization is workload divided
  by configured capacity. Therefore 24h capacity and 36h work means 0h
  available, -12h headroom, and 150% utilization.
...
## Approved corrective implementation sequence — generalized chat evidence

The detailed, executable test-first sequence is
`docs/superpowers/plans/2026-08-20-general-conversational-evidence-agent.md`.
It supersedes any earlier Task 4 chat instructions that make a fixed intent or
specialist route authoritative. Implement it in this order:

1. Define strict `EvidencePlan`, `MissingData`, capacity, and
   `UniversalEvidenceBundle` contracts.
2. Add Bedrock evidence-category planning and generic structured reference
   resolution.
3. Add the allowlisted read collector registry and deterministic capacity/risk/
   candidate derivation.
4. Rewire normal LangGraph chat to the generalized path; keep intent labels only
   as telemetry.
5. Add generic grounding/completeness, twenty unseen phrasings, multi-turn
   conversations, full/lint, and real MCP transport verification.

The Jira reassignment proposal, approval, execution, verification, and audit
workflow is excluded from this generalized chat planner and remains unchanged.
...
## Task 0.3: Validate Amazon Bedrock access and chat-failure assumptions

**Classification:** Security/Hardening

**Objective:** Verify the chosen AWS region exposes an approved Bedrock model,
grounded plain-text generation, typed transient failures, and persistent
browser retry behavior.

**Why this task is needed:** Every successful chat response depends on Bedrock;
deployment must prove access and safe unavailability without weakening scoring
or write safety.
...
- Validation checklist for model access, IAM denial, timeout/throttling,
  plain-text response, regional availability, bounded server attempts,
  persistent cancellable browser retry, and absence of sensitive prompt
  content.

**Detailed implementation steps:**

- [ ] Select one Bedrock model available in the deployment region and record its exact model ID.
- [ ] Invoke it with minimized evidence and validate a grounded plain-text answer.
- [ ] Deny model permission temporarily and confirm the permanent authorization
  failure returns a safe error and correlation ID without persistent retry.
- [ ] Inject retryable timeout, throttling, temporary 5xx, and open-circuit
  outcomes and confirm the browser keeps the same turn pending until Bedrock
  answers or the manager cancels/leaves.
...
jq -e '.model_id != "" and .region != "" and .plain_text_verified == true and .retry_exhaustion_verified == true' docs/validations/bedrock-model.json
```

**Expected output or observable result:** The selected model is listed and both validation booleans are true.

**Validation gate:** Cloud/security reviewer accepts the model, region, IAM
boundary, grounded output, and retry-exhaustion behavior.
...
**Objective:** Generate every successful final chat answer as grounded Bedrock
plain text over a canonical validated evidence bundle.
...
- Create: `services/agent-api/src/agent_api/llm/{protocol,bedrock,schemas,grounding}.py`
...
- Plain-text generation, Bedrock-invocation metadata, unknown Jira claim,
  changed score, invented candidate, malicious Jira content, timeout,
  throttling, 5xx, circuit open, IAM denial, empty/malformed output, bounded
  server attempts, persistent cancellable browser retry, and permanent-error
  termination. Every successful chat test asserts Bedrock was called;
  deterministic comment-signal classification remains separate.
...
- [ ] Keep typed schemas for evidence/internal steps, but use plain text for the
  final Bedrock generation call.
- [ ] Validate task/employee claims and compare any returned scores with
  immutable evidence before displaying the answer.
- [ ] Add bounded time/token budgets, at most two additional transient retries,
  exponential backoff with jitter, and a dependency-specific circuit breaker.
- [ ] Return typed retryability metadata after each bounded server attempt;
  never generate a deterministic final chat answer. The browser, implemented
  in Task 4.3, owns persistence across retryable transient failures.
- [ ] Emit one correlated lifecycle record across request/conversation/project,
  detected evidence needs, selected tools and safe arguments, MCP timing and
  outcome, Bedrock model/timing/AWS request ID/token counts/stop reason/outcome,
  parse and grounding results, and final status. Log unexpected stack traces
  server-side without exposing them to the UI.
- [ ] Classify Jira timeout/auth/rate-limit, Bedrock timeout/throttling/5xx,
  malformed or empty model output, parse, graph-state, reference-resolution,
  grounding, and internal failures separately.
...
**Expected output or observable result:** Mocked Bedrock produces every
successful chat answer; invalid claims are rejected and exhausted failures
return the safe error without changing scores.
...
## Task 4.2: Implement the evidence-routing Supervisor workflow

**Classification:** MVP

**Objective:** Implement LangGraph routing that optimizes known intents while
continuing arbitrary project questions through a general evidence-plus-Bedrock
path, with no conversational write path.
...
- Known intents, free-form project questions, unrelated questions, malformed MCP output,
...
- Prove mocked Bedrock is invoked for every successful chat turn: done-task
  lookup, WFD-7 follow-up without WFD-1, deterministic employee risk levels,
  risk urgency, insufficient vacation evidence, and ambiguous multi-task
  clarification.
- Prove Jira timeout and Bedrock throttling recover within three server-side
  attempts; exhausted transient Bedrock failures return typed retryability
  metadata for the browser without generating fallback prose.
...
- [ ] Retain known intent categories and per-tool allowlists, but add a general
  project-question route so classification never acts as a capability whitelist.
...
- [ ] Store bounded structured conversation references: last issues,
  employees, intent, evidence-bundle reference, and relevant messages.
- [ ] Resolve unambiguous follow-ups deterministically and pass ambiguous
  alternatives to Bedrock for a natural clarification question.
- [ ] Add graph deadlines, maximum steps, maximum tool calls, and graceful termination.
- [ ] Build one canonical evidence bundle with only relevant project,
  employee, issue, dependency, comment, risk, capacity, history, and missing-data
  sections, then always invoke Bedrock for final prose.
...
grounded Bedrock answer for known, free-form, insufficient-data, clarification,
and project-boundary questions; write requests remain outside chat.
...
- Bounded context IDs, grounded Bedrock text, stale warning, persistent
  transient Bedrock waiting and cancellation, free-form request, keyboard/screen-reader
...
- [ ] Keep one chat turn pending across retryable Bedrock timeout, throttling,
  temporary 5xx, and open-circuit responses. Display `Bedrock is still
  working...`, retry with capped exponential backoff and jitter, and preserve
  the exact question and structured context until a grounded Bedrock answer
  succeeds.
- [ ] Cancel the pending retry loop on a new question, Clear, project change,
  explicit Cancel, or page departure. Never append a transient-unavailable
  assistant message or retry a nonretryable failure.
...
- Make each specialist unavailable in turn and prove the Supervisor can gather
  remaining evidence and Bedrock explicitly identifies missing sources;
  write-oriented preparation remains closed.
...
- [ ] Add a specialist-unavailable evidence path that labels omissions and
  still invokes Bedrock when sufficient evidence remains.
- [ ] Retain the core Supervisor as the general evidence-routing path; it never
  generates final prose and cannot bypass Bedrock for successful chat.
...
handoffs among the five named agents; missing specialist evidence is explicit
and every successful final response remains Bedrock-generated.
...
- Seeded detection through verified reassignment, stale rejection, Bedrock
  transient-retry recovery and manager cancellation,
...
- [ ] Implement readiness checks for services, dev manager API key,
  lightweight UI, Bedrock grounded chat and retry-exhaustion handling, Jira MCP,
  seeded dataset, report, email,
  dashboards, and CI.
...
- [ ] On ambiguity, require `uncertain`, stop automatic action, preserve
  evidence, and select the prepared backup demonstration.
...
- [x] Task 4.2 implements the mandatory pre-authenticated Supervisor workflow;
  Task 4.4 adds the four selected extra-credit specialists with explicit
  missing-evidence handling. MCP servers remain tools and Workforce Risk MCP retains
...
- [x] Every successful chat response is Bedrock-generated plain text over
  validated evidence; general project questions are not blocked by a finite
  intent whitelist; retryable Bedrock exhaustion keeps the browser turn
  pending; permanent failures return a safe correlation-ID error.
